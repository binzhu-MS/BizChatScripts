...
# API modes for different model types
API_MODE_CHAT = "chat/completions"
API_MODE_TEXT = "text/completions"
...
class LLMAPI:
    ...
    _API_CHAT_COMPLETIONS = API_MODE_CHAT
    _DEFAULT_ENDPOINT = "https://fe-26.qas.bing.net/sdf/"

    def __init__(self, endpoint=None, auth=None, retries=3, api_mode=None):
        """
        ...
        Args:
            ...
            api_mode: API mode - 'chat/completions' or 'text/completions' (optional, defaults to 'chat/completions')
        """
        self.endpoint = endpoint or self._DEFAULT_ENDPOINT
        self.auth = auth or self._get_default_auth()
        self.retries = retries
        self.api_mode = api_mode or self._API_CHAT_COMPLETIONS

    def _get_default_auth(self):
        """Get default authentication from the local auth module."""
        try:
            from . import auth
        ...
        for attempt in range(self.retries):
            try:
                ...
                if e.response.status_code == 429:  # Rate limit
                    ...
                    logger.warning(
                        f"Rate limited. Retry {attempt+1}/{self.retries}. Sleeping {sleep_time}s."
                    )
                ...
                sleep_time = 5
                logger.warning(
                    f"Network error. Retry {attempt+1}/{self.retries}. Sleeping {sleep_time}s."
                )
                time.sleep(sleep_time)
    ...
    def send_request(self, model_name, request_data, chat_completion=True):
        ...
        # Use configurable API mode or fallback to default
        if chat_completion:
            api_endpoint = self.api_mode
        else:
            api_endpoint = API_MODE_TEXT  # Non-chat completion mode

        endpoint = f"{self.endpoint.rstrip('/')}/{api_endpoint}"
