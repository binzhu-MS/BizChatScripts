    ...
    try:
        ...
        if os.path.exists(excel_file):
            ...
            with open(excel_file, 'rb') as f:
                ...
                print(f"First 32 bytes (ascii, errors ignored): {repr(header[:32].decode('ascii', errors='ignore'))}")
                ...
                if header.startswith(b'PK'):
                    ...
                    print("File format not recognized - may be corrupted or not an Excel file")
                    print(f"Full header analysis needed. First 64 bytes: {header[:64].hex()}")
                
                # Look for Excel-specific strings
                excel_indicators = [b'Excel', b'Workbook', b'Sheet', b'Microsoft']
                found_indicators = [indicator for indicator in excel_indicators if indicator in header]
                if found_indicators:
                    print(f"Found Excel indicators: {found_indicators}")
                else:
                    print("No Excel indicators found in header - file may not be a valid Excel file")
        ...
        if header.startswith(b'PK'):
            ...
            try:
                ...
                print(f"Direct xlrd with ignore_workbook_corruption succeeded: {len(sheets)} sheet(s)")
                return sheets
            except Exception as xlrd_alt_error:
                print(f"Direct xlrd with ignore_workbook_corruption failed: {xlrd_alt_error}")
            ...
            try:
                ...
                with pd.ExcelFile(excel_file, engine='openpyxl') as xls:
                    sheets = [str(sheet) for sheet in xls.sheet_names]
                    print(f"Successfully read {len(sheets)} sheet(s) with openpyxl on OLE2 file")
                    return sheets
            except Exception as openpyxl_error:
            ...
            try:
                ...
                with pd.ExcelFile(excel_file) as xls:
                    sheets = [str(sheet) for sheet in xls.sheet_names]
                    print(f"Successfully read {len(sheets)} sheet(s) with pandas default engine")
                    return sheets
        ...
        for engine in engines_to_try:
            try:
                ...
                with pd.ExcelFile(excel_file, engine=engine) as xls:
                    sheets = [str(sheet) for sheet in xls.sheet_names]
                    print(f"Successfully read {len(sheets)} sheet(s) with {engine_name} engine")
                    return sheets
            ...
            for ignore_corruption in [False, True]:
                for formatting_info in [False, True]:
                    try:
                        ...
                        sheets = [str(sheet) for sheet in wb.sheet_names()]
                        print(f"Direct xlrd succeeded (ignore_corruption={ignore_corruption}, formatting_info={formatting_info}): {len(sheets)} sheet(s)")
                        return sheets
                    except Exception:
                        continue
        ...
        print("All Excel reading methods failed - file may be corrupted, password-protected, or in an unsupported format")
        print("Suggestions:")
...
def convert_excel_to_tsv(
    ...
    try:
        ...
        print(f"Found {len(sheet_names)} sheet(s): {', '.join(str(name) for name in sheet_names)}")
        ...
        if all_sheets:
            ...
            if sheet_name in sheet_names:
                ...
                print(f"Error: Sheet '{sheet_name}' not found. Available sheets: {', '.join(str(name) for name in sheet_names)}")
                return False
        else:
        ...
        for sheet in sheets_to_convert:
            ...
            try:
                ...
                try:
                    ...
                    print(f"  COM succeeded: Loaded {len(df)} rows, {len(df.columns)} columns")
            ...
            if all_sheets:
                ...
                safe_sheet_name = "".join(c for c in str(sheet) if c.isalnum() or c in (' ', '-', '_')).strip()
